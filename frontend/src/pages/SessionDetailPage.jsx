...
export default function SessionDetailPage() {
  ...
  if (error)   return <div className="session-detail-error" role="alert">Error: {error}</div>;
  ...
  return (
    <div className="session-detail">
      ...
      <div className="session-detail__metrics">
        <MetricCard label="Distance" value={m?.total_distance_m != null ? `${m.total_distance_m} m` : '—'} />
        <MetricCard label="Max Speed" value={m?.max_speed_kmh != null ? `${m.max_speed_kmh} km/h` : '—'} />
        <MetricCard label="Sprints" value={m?.sprint_count ?? '—'} />
        <MetricCard label="Player Load" value={m?.player_load != null ? m.player_load.toFixed(1) : '—'} />
      </div>
